...
class Sarsa_Function_Approximation:
    def __init__(
        self,
    ...
    def evaluation(self, max_steps: int = 200):
        ...
        s, steps = 0, 0
        a, traj_explored = self.sample_epsilon_greedy_action(s, self.epsilon)
        ...
        while not done and steps < max_steps:
            ...
            if done:
                ...
                with torch.no_grad(): # partial gradient for this update - standard practice
                    ...
                    a_dash, is_step_exploration = self.sample_epsilon_greedy_action(s_dash, self.epsilon)
                    traj_explored = traj_explored or is_step_exploration

                    s_dash_tensor = torch.nn.functional.one_hot(torch.tensor(s_dash), num_classes=self.env.n_states).to(torch.float32).unsqueeze(0)
                    q_s_dash_a_dash = self.q_net(s_dash_tensor).gather(dim=1, index=torch.tensor([a_dash], dtype=torch.long).unsqueeze(0))
                    # TD Target for Sarsa = r + gamma * Q_pi(s_dash, a_dash, w_k)
                    td_target = r_tensor + self.gamma * q_s_dash_a_dash
...
if __name__ == "__main__":
    ...
    sarsa_Function_Approximation = Sarsa_Function_Approximation(
        env,
        epsilon=0.99,
        epsilon_decay=0.999,
        epsilon_min=0.05,
    ...
    print("**************************************************")
    # print(sarsa_Function_Approximation.env.Q)

    print(f"Number of times goal was touched: {sarsa_Function_Approximation.logger['num_times_goal_touched']}")    

    env.plot_training_stats(sarsa_Function_Approximation.logger, smooth_window=50)
    env.plot_q_heatmap(title="Q-Value Heatmap (final Q)", show=False)
    plt.title("Sarsa w Function Approximation")
    plt.show()
